...
extern atomic64_t total_time_taken_for_all_exits;
extern u32 number_of_exits_in_one_exit_number[69];
extern atomic64_t time_of_one_exit[69];
...
static int __vmx_handle_exit(struct kvm_vcpu *vcpu, fastpath_t exit_fastpath)
{
	...
	if (exit_reason.basic <= 69) {
		number_of_exits_in_one_exit_number[exit_reason.basic]++;
	}
	...
	if (!kvm_vmx_exit_handlers[exit_handler_index])
		goto unexpected_vmexit;
	time_taken=rdtsc() - starting_time;	
	atomic64_add(time_taken, &total_time_taken_for_all_exits);

	exit_time_in_one_reason(exit_reason.basic,time_taken);
